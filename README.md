...
4. Ответьте на вопросы агента: название, автор, группа, стиль, обязательные и необязательные цели, daily/weekly квесты, passive/legendary идеи, звания и предметы экипировки.
5. Проверьте preview-таблицы: названия, условия, очки, обязательность, звания и reward items.
6. Попросите агента вывести финальный JSON content pack и текст для `README.md` кампании. Для кампании это всегда один JSON-файл: основные достижения, daily/weekly, passive hidden, legendary, звания и предметы находятся вместе.
...
- `campaign.json` - один `packType: "campaign"` JSON для экспорта в игру. В нем находятся основные достижения, daily/weekly квесты, passive hidden achievements, campaign legendary achievement, optional global legendary achievements, звания, предметы экипировки и автор кампании.
- `README.md` - человекочитаемое описание кампании: что это за кампания, кто автор, какая идея, какие квесты и достижения есть, какие условия у каждого пункта, сколько очков дается, какие звания и предметы открываются.
...
- В `README.md` кампании человекочитаемо описаны идея, автор, квесты, достижения, условия, очки, звания и предметы.
...
Основной путь для maintainer-а для кампаний без предметных изображений:

1. Открыть Filament admin.
2. Перейти в `Content -> Campaign Pack Importer`.
3. Загрузить `campaign.json` и иконку кампании.
4. Нажать `Проверить`.
5. Просмотреть отчет: кампания, автор, основные квесты, daily/weekly, скрытые достижения, легендарные достижения, звания, предметы, ошибки и предупреждения.
...
Если pack содержит `equipmentItems` с новыми `imageFileName`, используйте CLI-импорт или заранее положите файлы предметов рядом с JSON на сервере: importer ищет item images в той же папке, где лежит `campaign.json`.
...
- один JSON импортирует кампанию, основные достижения, daily/weekly, passive hidden, legendary, global legendary, звания и equipment items;
