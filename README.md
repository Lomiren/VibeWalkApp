...
- `equipmentCollections` - комплекты вещей, например “Комплект ведьмака”. Это группировка для магазина и сундука героя.
- `equipmentItems` - предметы экипировки, которые можно выдать за достижение или квест через `rewardEquipmentItem`.

## Как использовать ИИ агента

1. Откройте новый чат с ИИ агентом.
2. Вставьте или прикрепите содержимое `UserQuests/AI_AGENT_INSTRUCTIONS.md` как постоянную инструкцию для русскоязычного чата. Для англоязычного чата используйте `UserQuests/AI_AGENT_INSTRUCTIONS.en.md`.
3. Первым сообщением дайте агенту тему кампании и попросите провести интервью.
4. Ответьте на вопросы агента: название, автор, группа, стиль, обязательные и необязательные цели, daily/weekly квесты, passive/legendary идеи, звания, предметы экипировки и комплекты.
5. Проверьте preview-таблицы: названия, условия, очки, обязательность, звания, reward items и комплекты вещей.
6. Попросите агента вывести финальный JSON content pack и текст для `README.md` кампании. Для кампании это всегда один JSON-файл: основные достижения, daily/weekly, passive hidden, legendary, звания, комплекты и предметы находятся вместе.
...
- `campaign.json` - один `packType: "campaign"` JSON для экспорта в игру. В нем находятся основные достижения, daily/weekly квесты, passive hidden achievements, campaign legendary achievement, optional global legendary achievements, звания, комплекты вещей, предметы экипировки и автор кампании.
- `README.md` - человекочитаемое описание кампании: что это за кампания, кто автор, какая идея, какие квесты и достижения есть, какие условия у каждого пункта, сколько очков дается, какие звания, комплекты и предметы открываются.
- Иконка кампании - PNG или WebP файл, имя которого совпадает с `campaign.iconFileName` в JSON. Нормальный размер для импорта - `256x256` или больше; `512x512` не требуется. Если иконки пока нет, все равно укажите это в PR и в `README.md`.
- Изображения предметов - PNG или WebP файлы рядом с `campaign.json`. Имя каждого файла должно совпадать с `equipmentItems[].imageFileName`.
...
- комплекты вещей, если они есть;
- предметы экипировки, их слоты, редкость и какие достижения/квесты их выдают;
- статус иконки.
...
- Основные, пассивные и легендарные достижения дают от `5` до `50` очков включительно. Значение должно быть кратно `5`.
...
- Комплекты вещей описываются в top-level `equipmentCollections`, не внутри `campaign`.
- Предметы экипировки описываются в top-level `equipmentItems`, не внутри `campaign`.
- Если предмет входит в комплект, укажите `equipmentItems[].collection`. Значение должно ссылаться на `equipmentCollections[].id` из этого pack или на уже существующий активный комплект.
- Достижение или daily/weekly quest может выдать предмет через `rewardEquipmentItem`. Значение должно ссылаться на `equipmentItems[].id` из этого pack или на уже существующий активный предмет.
- Не вставляйте изображения в JSON. Если есть иконка, укажите имя файла в `iconFileName` и приложите файл отдельно. Если есть предметы, укажите файлы в `equipmentItems[].imageFileName` и приложите их отдельно.
- PNG/WebP `256x256` считается нормальным размером для иконки кампании и предметов. Максимальный размер файла - 2 MB. `imageFileName` должен быть только именем файла, без папок.

## Комплекты и предметы экипировки

Предметы создаются как server content items. Они не попадают в инвентарь пользователя сразу после импорта: игрок получает предмет только когда открывает достижение или квест с `rewardEquipmentItem`.

Комплект (`equipmentCollection`) - это server content grouping для вещей одной темы. Комплект не является покупкой одним кликом: в магазине и сундуке герой видит предметы, сгруппированные по комплекту, но каждая вещь покупается или выдается отдельно.

Поля комплекта:

| Поле | Обязательно | Описание |
| --- | --- | --- |
| `id` | Да | `snake_case` alias комплекта. |
| `localizations.ru/en.name` | Да | Название комплекта. |
| `localizations.ru/en.description` | Нет | Описание комплекта. |
...
Поддержанные `rarity` values:

| rarity | Смысл |
| --- | --- |
| `common` | Обычный серый предмет |
| `rare` | Редкий зеленый предмет |
| `legendary` | Легендарный оранжевый предмет |
| `epic` | Эпический фиолетовый предмет |

Поля предмета:

| Поле | Обязательно | Описание |
| --- | --- | --- |
| `id` | Да | `snake_case` alias предмета. |
| `slot` | Да | Один из поддержанных слотов. |
| `rarity` | Да | Один из поддержанных rarity. |
| `imageFileName` | Да | PNG/WebP файл рядом с `campaign.json`, без папок в имени. |
| `collection` | Нет | Alias комплекта из `equipmentCollections[].id` или существующий активный комплект. |
| `softPrice` | Нет | Цена в монетах, если предмет будет доступен в магазине. Для reward-only предмета можно не задавать. |
| `isStoreVisible` | Нет | `true`, если предмет должен быть виден в магазине. Для reward-only предмета обычно `false` или отсутствует. |
| `localizations.ru/en.name` | Да | Название предмета. |
| `localizations.ru/en.description` | Нет | Описание предмета. |

Пример выдачи предмета:
...
Если нужна новая группа, укажите ее в идее PR. Перед импортом администратор должен отдельно создать и опубликовать глобальную группу с `kind = campaign_group`; importer не создает новые группы автоматически.
...
12. Нужны ли предметы экипировки? Для каждого предмета нужны slot, rarity, ru/en название, imageFileName и достижение/квест, который его выдает.
13. Есть ли иконка кампании и изображения предметов? Если да, приложите PNG/WebP отдельно и укажите имена файлов.
...
- Если pack содержит `equipmentItems`, рядом с JSON приложены все файлы из `equipmentItems[].imageFileName`.
- Если pack содержит `equipmentCollections`, каждый комплект используется хотя бы одним `equipmentItems[].collection` или явно отмечен как задел на будущие вещи.
- В PR есть один `packType: "campaign"` JSON content pack.
- В `README.md` кампании человекочитаемо описаны идея, автор, квесты, достижения, условия, очки, звания, комплекты и предметы.
...
- Указан статус предметных изображений: приложены, нужна помощь или предметов нет.
...
- Main/passive/legendary имеют от `5` до `50` очков включительно, кратно `5`.
...
- Все `equipmentCollections` лежат на top-level JSON и имеют `ru/en` name.
- Все `equipmentItems` лежат на top-level JSON, имеют поддержанный slot/rarity и PNG/WebP `imageFileName`.
- Все `equipmentItems[].collection`, если указаны, ссылаются на комплект из pack или существующий активный комплект.
- Все `rewardEquipmentItem` ссылаются на предмет из pack или существующий активный предмет.

## Проверка и импорт maintainer-ом

### Admin UI

Основной путь для maintainer-а:

1. Открыть Filament admin.
2. Перейти в `Content -> Campaign Pack Importer`.
3. Загрузить `campaign.json`, иконку кампании и все PNG/WebP файлы из `equipmentItems[].imageFileName`, если pack содержит предметы.
4. Нажать `Проверить`.
5. Просмотреть отчет: кампания, автор, основные квесты, daily/weekly, скрытые достижения, легендарные достижения, звания, комплекты, предметы, ошибки и предупреждения.
6. Если статус `Importable`, нажать `Импортировать`.

После импорта кампания сразу попадает в published content tables. Отдельный draft/publish шаг для пользовательского campaign pack сейчас не используется.

### CLI

Maintainer проверяет пользовательский pack backend-командой:

```bash
php artisan user-content:import-campaign quests/<campaign_id>/campaign.json --icon=quests/<campaign_id>/<icon>.png --dry-run
```

Если отчет показывает `Importable: yes`, pack можно импортировать:
...
- без `--apply` команда только валидирует JSON и печатает отчет;
- все файлы из `equipmentItems[].imageFileName` должны лежать в той же папке, что и `campaign.json`;
- `campaign.group` должен уже существовать в базе как активная глобальная группа;
- один JSON импортирует кампанию, основные достижения, daily/weekly, passive hidden, legendary, global legendary, звания, equipment collections и equipment items;
- иконка копируется в public content storage и становится server-managed campaign icon;
- изображения предметов копируются в public content storage и становятся server-managed equipment item images;
- 256x256 считается нормальным размером иконки; меньше 256x256 не принимается;
- импорт не меняет пользовательский progress, completions, points, wallet или entitlements.
